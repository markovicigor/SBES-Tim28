...
namespace SecurityManager
{
    public class ServiceCertValidator : X509CertificateValidator
    {
        ...
        public override void Validate(X509Certificate2 certificate)
        {
            ...
            if (certificate.Issuer.Split('=')[1] != "SYSLOG_CA") // CN=SYSLOG_CA  ----> CN SYSLOG_CA ---->[1] --> SYSLOG_CA
            {
                throw new Exception("Sertifikat mora biti izdat od strane SYSLOG_CA.");
            }
            else
            {
                TimeSpan dt = certificate.NotAfter.Subtract(DateTime.Now);

                if ((float)dt.Days / 365 < 3)
                {
                    throw new Exception("Sertifikat mora biti validan duze od 3 godine.");
                }
            }
        }
    }
